...
"""
    ...
    def _generate_pdf(title: str, content: str) -> bytes:
        ...
        if WEASYPRINT_AVAILABLE:
            # Use WeasyPrint for high-quality PDFs
            html_content = f"""
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                <div class="header">
                    <h1>{title}</h1>
                    <p style="color: #666; font-size: 10pt;">Generated on {datetime.now().strftime('%d %B %Y')}</p>
                </div>
                <div class="content">{DocumentService._format_content_for_html(content)}</div>
                <div class="footer">
                    This document was generated by EnterprateAI
                </div>
            </body>
            </html>
            """
